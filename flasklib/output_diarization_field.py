...
class PlotDiar(object):
    ...
    """
    def __init__(self, map=None, title='', pick=False, vgrid=False, size=(18, 9)):
...
class OutputDiarizationField(OutputField):
    def __init__(self, field_name, default):
        self.value = None
        super(OutputDiarizationField, self).__init__(field_name, default)

    def set_output(self, value):
        self.value = value

    def generate_inputs(self):
        diar = PlotDiar(self.value)
        diar.draw()
        output = BytesIO()
        FigureCanvas(diar.fig).print_png(output)
        output.seek(0)
