...
class SpeakerMap(dict):
    """
    {
        0: [{'start': 0, 'stop': 100}, {'start': 200, 'stop': 300}],
        1: [{'start': 100, 'stop': 200}, {'start': 700, 'stop': 1200}],
        2: [{'start': 300, 'stop': 400}, {'start': 1200, 'stop': 1500}],
        3: [{'start': 400, 'stop': 700}]
    }
    """
    def __init__(self):
        super(SpeakerMap, self).__init__()

    def add_spk_period(self, spk_period):
        """
        Example:
            >> spk_period = {spk_key: (10, 100)}
            >> m.add_spk_period(spk_period)
        ...
        self.setdefault(key, []).append(timeDict)
        return self


class PlotDiar(object):
    """
