...
class SpeakerDiarization(object):
    def __init__(self, embedding_per_second=1.0, overlap_rate=0.5):
        ...
        self.network_eval = network_eval
        self.uisrnnModel = uisrnnModel
    ...
    def predict(self, wav_path):
        specs, intervals = load_data(wav_path,
                                     embedding_per_second=self.embedding_per_second,
        ...
        predicted_label = self.uisrnnModel.predict(feats,
                                                   self.inference_args)

        time_spec_rate = 1000 * (1.0 / self.embedding_per_second) * (1.0 - self.overlap_rate)
        center_duration = int(1000 * (1.0 / self.embedding_per_second) // 2)
    ...
    def predict_and_visualize(self, wav_path):
        speakerSlice = self.predict(wav_path)
